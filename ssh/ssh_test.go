...
var _ = Describe("SSH with private key and password", func() {
	...
	ssh := SSHArguments{Command: "ifconfig", Username: "admin1", Host: "192.168.1.88", Password: "admin", PrivateKey: privateKey}
...
var _ = Describe("SSH with private key", func() {
	...
	ssh := SSHArguments{Command: "ifconfig", Username: "admin1", Host: "192.168.1.88", PrivateKey: privateKey}
	requestBody := new(bytes.Buffer)
	errr := json.NewEncoder(requestBody).Encode(ssh)
	if errr != nil {
		log.Fatal(errr)
